...
*' buildings bounds in first calibration iterations for Germany to help calibration
sm_tmp = 5;  !! last iteration with bounds on buildings
loop(regi$(sameAs("DEU",regi)),
loop (ppfen_buildings_dyn36(in),
  vm_cesIO.lo(t_29(t),regi_dyn29(regi),in)
  = max(
      ( pm_cesdata(t,regi,in,"quantity")
        !! goes from 0.95 to 1e-12 in -0.2 steps
...
*' more generous calibration industry bounds for Germany to accomodate changes in feedstock shares
loop(regi$(sameAs("DEU",regi)),
loop (pf_industry_relaxed_bounds_dyn37(in),
  vm_cesIO.lo(t_29(t),regi_dyn29(regi),in)
  = max(
