...
*'  The overall tax revenue is the sum of two main components, each consisting of various subcomponents:
...
*'  Note that the distinction between 1. and 2. is not always clear and can depend on region, sector and scenario (e.g. carbon taxes are implemented in some regions and sectors as real taxes, but in others serve as a proxy for other climate policy measures)
*'  Each subcomponent has the following similar structure:
...
*'  Further explanations on this iterative taxation mechanism can be found in the following tutorial: 16_taxation_mechanism_toy_model.md
***---------------------------------------------------------------------------
q21_taxrev(t,regi)$(t.val ge max(2010,cm_startyear))..
  vm_taxrev(t,regi)
  =e=
    v21_taxrevReal(t,regi)
  + v21_taxrevPseudo(t,regi)
;

*'  1. Net revenue from real taxes and subsidies, v21_taxrevReal, that model an actual monetary transaction (e.g. FE taxes and carbon taxes)
q21_taxrevReal(t,regi)$(t.val ge max(2010,cm_startyear))..
  v21_taxrevReal(t,regi)
  =e=
    v21_taxrevGHG(t,regi)
  - v21_taxrevCDR(t,regi)$(cm_iterative_target_adj ge 10) 
...
v21_taxrevGHG(t,regi) =e= pm_taxCO2eqSum(t,regi) * (vm_co2eq(t,regi) 
  	                                        + vm_emiCdrAll(t,regi)$((cm_iterative_target_adj ge 10))
                                            - vm_emiMacSector(t,regi,"co2luc")$(cm_multigasscen ne 3))
                          - pm_taxrevGHG0(t,regi)
;

!!                                            + vm_emiCdrNovel(t,regi)$((cm_iterative_target_adj ge 10) AND (cm_CDRtypesSeparate eq 2))
!!                                            + vm_emiCdrNovel_NoInd(t,regi)$((cm_iterative_target_adj ge 10) AND (cm_CDRtypesSeparate eq 3)) 

***---------------------------------------------------------------------------
*'  Calculation of CDR revenues: This is only active if
*'  cm_iterative_target_adj ge 10. Otherwise pm_taxCDR is set to zero.
*'  Documentation of overall tax approach is above at q21_taxrev.
***---------------------------------------------------------------------------
...
***---------------------------------------------------------------------------
*'  Calculation of CO2 taxes on land use co2 emissions: tax rate (combination of 4 components) times land use co2 emissions
*'  Documentation of overall tax approach is above at q21_taxrev.
***---------------------------------------------------------------------------
